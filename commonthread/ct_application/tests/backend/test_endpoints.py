...
def seed():
    ...
    proj1 = Project.objects.create(
        org=org1, name="Campus Tales", curator=alice, date=datetime.date(2025, 4, 1)
    )
    proj_edit_delete = Project.objects.create(
        org=org3, name="To Be Edited", curator=deleto, date=datetime.date(2025, 4, 7)
    )
...
def test_project_dashboard_ok(client, seed, auth_headers):
    ...
    r = client.get(
        f"/project/{p.id}",  # org1 hardcoded, just to test page working at all
        **auth_headers(),
    )
...
def test_get_ml_status_ok_independent(client):
    ...
    project = Project.objects.create(
        org=org, curator=user, name="ML Test Project", date=datetime.date(2025, 5, 23)
    )
...
def mock_s3_presigned():
    return {"url": "https://fake-s3-url.com", "fields": {"key": "value"}}
...
def test_create_story_with_media(client, auth_headers, basic_story_payload):
    ...
    payload.update(
        {"audio_path": "user/123/audio.mp3", "image_path": "user/123/image.jpg"}
    )
...
def test_create_story_queue_failure(
    mock_queue_producer, client, auth_headers, basic_story_payload
):
...
def test_get_story_with_media(client, seed, monkeypatch, auth_headers):
    ...
    monkeypatch.setattr(
        settings, "CT_BUCKET_AUDIO", settings.CT_BUCKET_STORY_AUDIO, raising=False
    )
    monkeypatch.setattr(
        settings, "CT_BUCKET_IMAGES", settings.CT_BUCKET_STORY_IMAGES, raising=False
    )
...
def test_edit_story(client, seed, auth_headers_user3):
    ...
    payload = {
        "storyteller": "Testy",
        "curator": deleto.id,
        "text_content": "Content has been Edited",
        "proj": p.id,
    }
    r = client.post(
        f"/story/{s.id}/edit",
...
def test_expired_token_299(client, seed):
    ...
    hdrs = {"HTTP_AUTHORIZATION": f"Bearer {expired_access_token(alice.id)}"}
...
def test_project_forbidden_403(client, seed, auth_headers):
    ...
    r = client.get(f"/project/{p.id}", **hdrs)
